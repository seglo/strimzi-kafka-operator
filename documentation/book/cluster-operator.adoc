...
A `Kafka` resource is used for Kafka cluster configuration, and a `ConfigMap` is used for Kafka Connect cluster configuration.
The ConfigMaps for Kafka Connect clusters need to have a specific label which is, by default, `strimzi.io/kind=cluster` (as described <<Format of the cluster ConfigMap,later>>) that can be changed through a corresponding environment variable.

When a new desired resource (that is, a Kafka resource or a Kafka Connect ConfigMap) is created in the {ProductPlatformName} cluster, the operator gets the cluster configuration from the desired resource and starts creating a new Kafka or Kafka Connect cluster by creating the necessary other {ProductPlatformName} resources, such as StatefulSets, Services, ConfigMaps etc.
...
Finally, when the desired resource is deleted, the operator starts to un-deploy the cluster deleting all the related {ProductPlatformName}
resources.
...
if the operator is not running, or if a notification is not received for any reason, the desired resources will get out of sync
...
This provides a convenient mechanism for those resource to be labelled in whatever way the user requires.
...
spec:
  kafka:
    # ... configuration of replicas, image etc.
    config:
      num.partitions: 1,
...
Kafka cluster might not start or might become unstable. In such cases, the configuration in the `spec.kafka.config` object
...
http://zookeeper.apache.org/doc/r3.4.12/zookeeperAdmin.html[Zookeeper website]. An example `spec.zookeeper.config` object is provided
below.
...
NOTE:: The Cluster Operator doesn't validate the provided configuration. When invalid configuration is provided, the
Zookeeper cluster might not start or might become unstable. In such cases, the configuration in the `spec.zookeeper.config` object
...
.Example `ephemeral` storage for Kafka
[source,json]
----
spec:
  kafka:
...
.Example configuring Kafka with `persistent-storage` and 1Gi `size`
[source,json]
----
spec:
  kafka:
...
.Example configuring Kafka with `persistent-storage` using a storage class
[source,json]
----
spec:
  kafka:
...
Finally, a selector can be used in order to select a specific labeled persistent volume which provides some needed features (such as an SSD)

.Example configuring Kafka with "match labels" selector
[source,json]
----
spec:
  kafka:
...
Because {ProductName} uses the [JMX exporter](https://github.com/prometheus/jmx_exporter) in order to expose metrics on each node, the JSON string used for metrics configuration in the Kafka resource of Kafka Connect cluster ConfigMap reflects the related JMX exporter configuration file.
For this reason, you can find more information on how to use it in the corresponding GitHub repo.
...
It is possible to configure {ProductPlatformName} resource limits and requests on several containers using a JSON or YAML object.
...
The operator watches for ConfigMaps having the label `strimzi.io/kind=cluster` in order to find and get configuration for a Kafka Connect cluster to deploy.

In order to distinguish which "type" of cluster to deploy, the operator checks the `strimzi.io/type` label which can have one of the the following values :

* `kafka-connect`: the ConfigMap provides configuration for a Kafka Connect cluster deployment
* `kafka-connect-s2i`: the ConfigMap provides configuration for a Kafka Connect cluster deployment using Build and Source2Image features (works only with {OpenShiftName})

Whatever other labels are applied to the ConfigMap will also be applied to the {ProductPlatformName} resources making up the Kafka Connect cluster.
