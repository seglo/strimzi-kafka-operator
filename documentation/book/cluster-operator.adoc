...
NOTE:: The Cluster Operator doesn't validate keys or values in the provided `config` object.
...
Without this object, the Cluster Operator doesn't deploy the topic operator. It is still possible to deploy the topic
...
`image`:: 
The Docker image to use for the Kafka Connect workers. 
...
Kafka Connect cluster might not start or might become unstable. In such cases, the configuration in the `config` object
